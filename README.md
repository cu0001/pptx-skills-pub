...
> **Bob 向けセットアップガイド**  
> 自然言語のデザイン指示から PowerPoint（.pptx）ファイルを自動生成する Bob 向けスキルライブラリです。  
> デザインプロンプトをテーマトークンに変換し、PptxGenJS で安全に高品質なプレゼン資料を出力します。
...
- **最小限の依存関係** — 基本的な依存モジュールは `pptxgenjs` のみ。導入リスクを最小限に抑えられる
...
> [!IMPORTANT]
> Bob のスキルは **Advanced モード** でのみ動作します。  
> Bob Settings → Mode → Advanced に切り替えてください。
...
> [!TIP]
> 本プロジェクトは **`pptxgenjs` 1つのみ** で動作するように設計されています。余計なモジュールの導入や複雑な環境構築は不要です。
>
> | パッケージ | バージョン | 用途 |
> |---|---|---|
...
Bob のスキルは、**`.bob/skills/` 配下の `SKILL.md`** を読み込み、Bob 自身がいつ・どのようにスキルを使うかを自動判断します。
...
blue-modern 版だけで構いませんので renderers 採用から検討してください。
```
...
| 項目 | 詳細 |
|---|---|
| **モード** | **Advanced モード必須**。 Standard モードではスキルが起動しません。 |
| **色コード** | `#` が含まれていると生成エラーになる場合があります。 `ThemeEngine` が自動除去を試みますが、指定時は `6桁 HEX` を推奨します。 |
| **フォント** | 安全フォント (`Arial`, `Calibri`, `Meiryo`, `Yu Gothic` 等) 外を使用すると、自動でフォールバックされます。 |
| **アニメーション** | PptxGenJS の制限により、スライドアニメーションには対応していません。 |
| **ビジュアル確認** | 生成結果に対する画像解析（Visual QA）は行われません。そのため、文字重なり等が発生した場合は、手動または AI への再指示による調整が必要です。 |
...
本プロジェクトおよび依存モジュールを削除する場合は、以下の手順を実行してください。
...
本ソフトウェアは「現状のまま（AS IS）」提供され、明示的か暗示的かを問わず、その品質、性能、特定目的への適合性、または権利侵害の不在について、一切の保証（法的保証を含む）を負いません。本ソフトウェアの使用に起因して生じた直接的、間接的、偶発的、または特別な損害（利益の損失、データの損失、業務の中断等を含む）について、作者および著作権者は一切の責任を負いません。ご利用の際は、ユーザー自身の責任において十分な検証を行ってください。
